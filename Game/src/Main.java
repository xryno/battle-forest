...
public class Main {
    ...
    public static JFrame createGUI() {
        ...
        JButton[] buttonsArr = { attackButton, magicButton, backButton, fireButton, lightningButton, healButton,
                itemButton };
        for (JButton each : buttonsArr) {
            each.setPreferredSize(butDim);
        }

        JLabel[] labelsArr = { playerName, playerName2, monsterName, playerHPL, playerMPL, player2HPL, player2MPL,
                enemyHPL, combatMsgs, nameTitle, timeTitle, hpTitle, mpTitle, mhpTitle, monsterTitle, mtimeTitle };
        for (JLabel each : labelsArr) {
            each.setForeground(Color.white);
        }

        JLabel[] StatlabelsArr = { playerName, playerName2, monsterName, playerHPL, playerMPL, player2HPL, player2MPL,
                nameTitle, timeTitle };
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                ...
                if (val >= 100) {
                    ...
                    if (Monster.playerToAttack == 1) {
                        ...
                        combatMsgs
                                .setText(String.format("%s attacked %s for %d HP", monster.name, player.name, atkVal));
                    } else {

                        player2.hp -= atkVal;
                        enemyc.setBounds(25, 80, 300, 268);
                        combatMsgs
                                .setText(String.format("%s attacked %s for %d HP", monster.name, player2.name, atkVal));
                    }
                    progressBarE.setValue(0);
                    return;
                }
        ...
        attackButton.addActionListener(event -> {
            ...
            combatMsgs.setText(String.format("%s attacked %s for %d HP!", Player.active.get(0).name, monster.name,
                    Player.active.get(0).lastAtk));
        ...
        fireButton.addActionListener(event -> {
            ...
            if (Player.active.get(0).mp >= 50) {
                ...
                combatMsgs.setText(String.format("%s cast fire spell for %d HP!", Player.active.get(0).name,
                        Player.active.get(0).lastAtk));
        ...
        lightningButton.addActionListener(event -> {
            ...
            if (Player.active.get(0).mp >= 40) {
                ...
                combatMsgs.setText(String.format("%s cast lightning spell for %d HP!", Player.active.get(0).name,
                        Player.active.get(0).lastAtk));
